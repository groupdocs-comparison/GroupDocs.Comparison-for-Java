...
import com.groupdocs.ui.comparison.dropwizard.common.entity.web.*;
...
import com.groupdocs.ui.comparison.dropwizard.provider.FilesProvider;
import com.groupdocs.ui.comparison.dropwizard.service.ComparisonService;
import com.groupdocs.ui.comparison.dropwizard.service.ComparisonServiceImpl;
import com.groupdocs.ui.comparison.dropwizard.views.Comparison;
...
public class ComparisonResources extends Resources implements HttpSessionListener {
    ...
    @Produces(APPLICATION_JSON)
    @Consumes(APPLICATION_JSON)
    public Response loadDocumentDescription(@Session HttpSession httpSession, LoadDocumentRequest loadDocumentRequest) {
        try {
            final Object attribute = httpSession.getAttribute(SESSION_CACHE_KEY);
            if (attribute instanceof SessionCache) {
                final SessionCache sessionCache = (SessionCache) attribute;

                final LoadDocumentEntity loadDocumentEntity = comparisonService.loadDocumentDescription(loadDocumentRequest, sessionCache);
                return Response.ok(loadDocumentEntity).build();
            } else if (attribute != null) {
                throw new IllegalStateException("Unknown session cache object");
            }
            throw new IllegalStateException("Session cache object does not exist");
            ...
            return Response.status(403).entity(exceptionEntity).build();
        } catch (TotalGroupDocsException e) {
            logger.error("Loading document error!", e);
            // Message is correct to show it to user
            throw new TotalGroupDocsException(e.getMessage());
